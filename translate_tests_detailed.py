...
PROTOCOLS = ["Fujitsu", "Daikin", "Mitsubishi", "Panasonic", "Samsung", "LG",
             "Carrier", "Haier", "Hitachi", "Sharp", "Toshiba", "Vestel",
             "Whirlpool", "Electra", "Delonghi", "Corona"]
...
def extract_byte_arrays(test_body: str) -> List[Tuple[str, List[int]]]:
    ...
    pattern = re.compile(
        r'uint8_t\s+(\w+)\s*\[\s*(?:\d+|kFujitsu\w+)?\s*\]\s*=\s*\{([^}]+)\}',
        re.DOTALL
    )
...
def process_protocol(protocol: str) -> Tuple[int, int]:
    ...
    print(f"  Created {py_file} with {min(10, len(tests))} detailed tests (from {len(tests)} total)")

...
def sendPanasonic(address: int, data: int, nbits: int = kPanasonicBits,
...
def encodePanasonic(manufacturer: int, device: int, subdevice: int,
                    function: int) -> int:
    """
...
def decodePanasonic(results, offset: int = 1, nbits: int = kPanasonicBits,
                    strict: bool = True, manufacturer: int = 0x4004) -> bool:
    """
...
def sendPanasonicAC(data: List[int], nbytes: int = kPanasonicAcStateLength,
                    repeat: int = 0) -> List[int]:
    """
...
class IRPanasonicAc:
    ...
    def setModel(self, model: int) -> None:
        if model not in [kPanasonicDke, kPanasonicJke, kPanasonicLke,
                        kPanasonicNke, kPanasonicCkp, kPanasonicRkr]:
    ...
    def setMode(self, desired: int) -> None:
        ...
        if desired == kPanasonicAcFan:
            ...
            mode = desired
        elif desired in [kPanasonicAcAuto, kPanasonicAcCool,
                        kPanasonicAcHeat, kPanasonicAcDry]:
            mode = desired
    ...
    def getTemp(self) -> int:
        return GETBITS8(self.remote_state[14], kPanasonicAcTempOffset,
    ...
    def setTemp(self, celsius: int, remember: bool = True) -> None:
        ...
        setBits([self.remote_state[14]], kPanasonicAcTempOffset,
               kPanasonicAcTempSize, temperature)
    ...
    def setSwingHorizontal(self, desired_direction: int) -> None:
        if desired_direction not in [kPanasonicAcSwingHAuto, kPanasonicAcSwingHMiddle,
                                     kPanasonicAcSwingHFullLeft, kPanasonicAcSwingHLeft,
                                     kPanasonicAcSwingHRight, kPanasonicAcSwingHFullRight]:
    ...
    def setFan(self, speed: int) -> None:
        if speed in [kPanasonicAcFanMin, kPanasonicAcFanLow, kPanasonicAcFanMed,
                    kPanasonicAcFanHigh, kPanasonicAcFanMax, kPanasonicAcFanAuto]:
            setBits([self.remote_state[16]], kHighNibble, kNibbleSize,
    ...
    def _getTime(ptr: List[int]) -> int:
        result = (GETBITS8(ptr[1], kLowNibble, kPanasonicAcTimeOverflowSize) <<
    ...
    def getOffTimer(self) -> int:
        result = (GETBITS8(self.remote_state[20], 0, 7) << kNibbleSize) | \
                 GETBITS8(self.remote_state[19], kHighNibble, kNibbleSize)
        if result == kPanasonicAcTimeSpecial:
            return 0
        return result
    ...
    def getIon(self) -> bool:
        if self.getModel() == kPanasonicDke:
            return bool(GETBIT8(self.remote_state[kPanasonicAcIonFilterByte],
    ...
    def setIon(self, on: bool) -> None:
        if self.getModel() == kPanasonicDke:
            setBit([self.remote_state[kPanasonicAcIonFilterByte]],
                  kPanasonicAcIonFilterOffset, on)
            self.remote_state[kPanasonicAcIonFilterByte] = \
                [self.remote_state[kPanasonicAcIonFilterByte]][0]
...
def decodePanasonicAC(results, offset: int = 1, nbits: int = kPanasonicAcBits,
...
def sendPanasonicAC32(data: int, nbits: int = kPanasonicAc32Bits,
                      repeat: int = 0) -> List[int]:
    """
    Send a Panasonic AC 32/16bit formatted message.
...
class IRPanasonicAc32:
    ...
    def setMode(self, mode: int) -> None:
        if mode in [kPanasonicAc32Auto, kPanasonicAc32Cool, kPanasonicAc32Dry,
                   kPanasonicAc32Heat, kPanasonicAc32Fan]:
            self._.Mode = mode
        else:
    ...
    def setFan(self, speed: int) -> None:
        if speed in [kPanasonicAc32FanMin, kPanasonicAc32FanLow, kPanasonicAc32FanMed,
                    kPanasonicAc32FanHigh, kPanasonicAc32FanMax, kPanasonicAc32FanAuto]:
            self._.Fan = speed
        else:
...
def decodePanasonicAC32(results, offset: int = 1, nbits: int = kPanasonicAc32Bits,
                        strict: bool = True) -> bool:
    """
    Decode a Panasonic AC 32/16bit IR message.
    EXACT translation from IRremoteESP8266 IRrecv::decodePanasonicAC32
    """
    from app.core.ir_protocols.ir_recv import _matchGeneric, kHeader, kFooter, kUseDefTol, kMarkExcess
    ...
    if is_long:
        min_length = (kPanasonicAc32Sections * kPanasonicAc32BlocksPerSection *
                     ((2 * nbits) + kHeader + kFooter) - 1 + offset)
    else:
        min_length = ((kPanasonicAc32BlocksPerSection + 1) * ((4 * nbits) + kHeader) +
                     kFooter - 1 + offset)

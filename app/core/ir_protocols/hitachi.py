...
class IRHitachiAc1:
    ...
    def setMode(self, mode: int) -> None:
        if mode == kHitachiAc1Auto:
            ...
            # FALL THRU
        if mode in [kHitachiAc1Auto, kHitachiAc1Fan, kHitachiAc1Heat, kHitachiAc1Cool, kHitachiAc1Dry]:
            self._.Mode = mode
        else:
            self.setTemp(kHitachiAc1TempAuto)
    ...
    def getOnTimer(self) -> int:
        return reverseBits(
            (self._.OnTimerLow << 8) | self._.OnTimerHigh, kHitachiAc1TimerSize)
    ...
    def getOffTimer(self) -> int:
        return reverseBits(
            (self._.OffTimerLow << 8) | self._.OffTimerHigh, kHitachiAc1TimerSize)
...
class IRHitachiAc264(IRHitachiAc424):
    ...
    def setFan(self, speed: int) -> None:
        if speed in [kHitachiAc264FanMin, kHitachiAc264FanMedium, kHitachiAc264FanHigh, kHitachiAc264FanAuto]:
            self._.Fan = speed
        else:
...
class IRHitachiAc296:
    ...
    def setMode(self, mode: int) -> None:
        if mode in [kHitachiAc296Heat, kHitachiAc296Cool, kHitachiAc296Dehumidify, 
                    kHitachiAc296AutoDehumidifying, kHitachiAc296Auto]:
            self._.Mode = mode
...
def decodeHitachiAC(results, offset: int = 1, nbits: int = kHitachiAcBits, 
                    strict: bool = True, MSBfirst: bool = True) -> bool:
    """
    Decode the supplied Hitachi A/C message.
    ...
    if strict:
        if nbits not in [kHitachiAcBits, kHitachiAc1Bits, kHitachiAc2Bits, 
                         kHitachiAc264Bits, kHitachiAc344Bits]:
...
def decodeHitachiAc424(results, offset: int = 1, nbits: int = kHitachiAc424Bits, 
...
def decodeHitachiAc3(results, offset: int = 1, nbits: int = kHitachiAc3Bits, 
                     strict: bool = True) -> bool:
    """
    Decode the supplied Hitachi 15to27-byte/120to216-bit A/C message.
...
def decodeHitachiAc296(results, offset: int = 1, nbits: int = kHitachiAc296Bits, 
                       strict: bool = True) -> bool:
    """
    Decode the supplied Hitachi 37-byte A/C message.
